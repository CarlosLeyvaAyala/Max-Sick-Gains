...
; Loads the list of Known NPCs generated by _Max Sick Gains.exe_.
;
; This function saves the id of an `ActorBase` that will be used to rettrieve data found
; in `database.lua > database.npcs`.
;
; That `id` is passed to Lua by `Maxick_NPC._InitNpcData()`.
Function _LoadKnownNPCs()
  int knownNPCs = JValue.readFromFile("data/SKSE/Plugins/Maxick/known-npcs.json")
  int n = JArray.count(knownNPCs)
  int i = 0

  While (i < n)
    int npc = JArray.getObj(knownNPCs, i)
    int formId = JMap.getInt(npc, "formId")
    string esp = JMap.getStr(npc, "esp")
    int dbId = JMap.getInt(npc, "id")

    Form frm = Game.GetFormFromFile(formId, esp)
    If !frm
      string fullName = JMap.getStr(npc, "fullName")
      md.LogCrit( DM_Utils.LuaStr("maxick.ErrorKNpcFormNotFound", fullName, esp, formId, dbId) )
    Else
      Maxick_DB.FormSaveInt(frm, "knownNpcId", dbId)
    EndIf

    i += 1
  EndWhile

  ; Maxick_DB.SaveToFile("Known NPCs")
EndFunction
